...
public class HatManager : MonoBehaviour, IPointerClickHandler
{
    ...
    public IEnumerator RaiseHat(bool reportGuess = false)
    {
        ...
        gameObject.LeanMove(new Vector3(originalX, 1, 0), 0.5f);
        yield return new WaitForSeconds(0.5f);  // Wait for the animation above to finish

        yield return new WaitForSeconds(1.5f);  // Wait before starting the second animation

        gameObject.LeanMove(new Vector3(originalX, 0, 0), 0.5f);
        yield return new WaitForSeconds(0.5f);  // Wait for the second animation to end
